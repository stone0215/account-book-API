...
def init_dashboard_api(app):
        ...
        try:
            ...
            for key, groups in groupby(journals, lambda item: item.action_main_type):
                ...
                groupedJournalValues.append(
                    {"name": key, "value": abs(round(sum(spendings), 2))}
                )  # 取絕對值計算百分比
            ...
            if len(groupedJournalValues) > 0:
                floatValue = (
                    [
                        data
                        for data in groupedJournalValues
                        if data.get("name") == "Floating"
                    ][0]["value"]
                    if [
                        data
                        for data in groupedJournalValues
                ...
                output["workFreedom"] = (
                    round((passiveValue / (passiveValue + IncomeValue)) * 100, 2)
                    if passiveValue + IncomeValue > 0
                    else 0
                )
            ...
            for key, groups in groupby(debts, lambda item: item.dateString):
                balances = [
                    round(abs(item.balance) * item.fx_rate, 2) for item in list(groups)
                ]
                while mappedDate.strftime(strFormat) != key:
                ...
                groupedDebtValues.append(
                    {"dateString": key, "type": "debt", "value": sum(balances)}
                )
                mappedDate = mappedDate + (
                    relativedelta(months=1)
                    if type == "month"
            ...
            thisPeriodData = [
                data
                for data in groupedAssetValues
                if data.get("dateString") == lastPeriod
            ]
            assetValue = 0
            liabilityValue = 0
            ...
            if len(thisPeriodData) == 0:
                ...
                liabilityValue = [
                    data
                    for data in groupedDebtValues
                    if data.get("dateString") == lastPeriod
                ][0]["value"]

            thisNetAssetValue = assetValue - liabilityValue
            lastNetAssetValue = (
                groupedAssetValues[len(groupedAssetValues) - 2]["value"]
                - groupedDebtValues[len(groupedDebtValues) - 2]["value"]
            )
            ...
            for alarm in alarms:
                if alarm.alarm_type == "M":
                    for i in range(0, 6):
                        ...
                        # 目前要跑得月份小於到期日才加
                        if alarm.due_date == None or (
                            alarm.due_date
                            and month.strftime("%Y%m")
                            <= datetime.strptime(alarm.due_date, "%Y-%m-%d").strftime(
                                "%Y%m"
                            )
                        ):
                            output.append(
                                {
                                    "date": month.strftime("%m")
                                    + "/"
                                    + alarm.alarm_date,
            ...
            if result:
                ...
                return jsonify(
                    ResponseFormat.false_return(
                        ResponseFormat, None, "fail to add journal data"
                    )
                )
        except Exception as error:
            return jsonify(ResponseFormat.false_return(ResponseFormat, error))
        ...
        try:
            ...
            if target is None:
                ...
                if TargetSetting.update(TargetSetting):
                    ...
                    return jsonify(
                        ResponseFormat.false_return(
                            ResponseFormat, None, "fail to update account data"
                        )
                    )
        except Exception as error:
            return jsonify(ResponseFormat.false_return(ResponseFormat, error))
    ...
    def deleteTarget(id):
        ...
        if target is None:
            return jsonify(
                ResponseFormat.false_return(ResponseFormat, None, "找不到要删除的数据")
            )
        else:
            ...
            if TargetSetting.delete(TargetSetting, id):
                ...
                return jsonify(
                    ResponseFormat.false_return(ResponseFormat, None, "删除失败")
                )
