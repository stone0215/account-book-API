...
class StockJournal(db.Model):
    ...
    def query4Display(self, asset_id):
        ...
        sql.append("IFNULL(total_sell,0) AS sold_amount, ")
        sql.append(
            "(total_buy+IFNULL(total_stock_dividend,0)+IFNULL(total_sell,0)) AS hold_amount, "
        )
        ...
        sql.append(
            "IFNULL(total_cost,0) AS total_cost, IFNULL(total_cash_dividend,0) AS total_cash_dividend, total_buy "
        )
    ...
    def query4Summary(self, vestingMonth):
        ...
        sql.append(
            " Main.asset_name AS asset_name, Account.fx_code AS fx_code, IFNULL(buy_rate,1) AS fx_rate FROM Stock_Journal Journal "
        )
        sql.append(
    ...
    def output4View(self, stock):
        return {
            ...
            "buy_price": round(abs(stock.total_cost) / stock.hold_amount, 2)
            if stock.hold_amount != 0
            else 0,
            "sold_price": round(abs(stock.total_gain) / stock.sold_amount, 2)
